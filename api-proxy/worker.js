...
// ---------------------------------------------------------------------------
// ใช้วันที่ไทยให้ตรงกับฟิลด์ date ของแอป เพื่อไม่ให้บิลข้ามวันตอนเที่ยงคืน
...
async function buildDailyReport(env, injected) {
  ...
  const stock = await createFirestore(env, deps).listAll('stock').catch(() => []);
  ...
  const lines = [
    `สรุปวันนี้ · ${thaiDateLabel(new Date(deps.now()))}`,
    '',
