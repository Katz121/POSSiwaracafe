  ...
  return (
    <div className="w-80 lg:w-96 shrink-0 flex flex-col gap-4 bg-gray-900/50 rounded-[2.5rem] p-4 border border-gray-800 shadow-2xl">
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-800/50 pb-4 text-gray-400">
        ...
        <span className="bg-gray-800 text-emerald-400 px-3 py-1 rounded-full text-[11px] font-black border border-gray-700">{filtered.length}</span>
      </div>
      <div className="flex-1 overflow-y-auto space-y-5 pr-1 scrollbar-hide text-gray-200">
        {filtered.map(order => (
          <div key={order.id} className="bg-gray-800 border border-gray-700 rounded-[2rem] overflow-hidden shadow-xl relative animate-in zoom-in duration-300">
            <div className={`absolute top-0 left-0 px-4 py-2 text-[10px] font-black uppercase rounded-br-2xl z-20 ${order.isPaid ? 'bg-emerald-500 text-white' : 'bg-orange-500 text-white animate-pulse'}`}>
            ...
            <div className="p-5 bg-gray-900/50 flex flex-col gap-4">
              <div className="flex justify-between text-[11px] border-t border-gray-700 pt-4 font-bold">
              ...
              <div className="flex gap-3">
                <button onClick={() => onCancel(order.id)} className="flex-1 py-4 rounded-2xl text-[11px] font-black text-red-400 border border-red-900/30 hover:bg-red-500 hover:text-white transition-all uppercase active:scale-95">ลบ</button>
