        ...
        <div className="h-full bg-[var(--bg-primary)] flex flex-col overflow-hidden text-[var(--text-primary)] relative">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-6 space-y-6 md:space-y-8 lg:space-y-10 scrollbar-hide pb-24 md:pb-32">
                ...
                <Card padding="lg" className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <h2 className="font-bold">กำไรขั้นต้น</h2>
                        <p className="text-3xl font-bold text-[var(--accent-emerald)] num">฿{Math.round(profitability.grossProfit).toLocaleString()}</p>
                        <p className="text-sm text-[var(--text-muted)]">มาร์จิ้น {profitability.grossMargin}%</p>
                    </div>
                    <Button onClick={() => onNavigate('profitability')}>ดูละเอียด</Button>
                </Card>
